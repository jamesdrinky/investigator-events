...
function spotlightCard(ed: WeeklyEditorial) {
  ...
  // A "Watch" kicker switches the slot to a video treatment: the image renders
  // as a full-width 16:9 poster (linked) instead of the 56px logo square.
  const isVideo = ed.spotlightKicker?.trim().toLowerCase() === 'watch';
  if (isVideo && img) {
    return `
  <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:40px;">
    <tr><td style="padding:24px 22px;background:linear-gradient(135deg,#f5f3ff,#eff6ff);background-color:#f5f3ff;border:1px solid #ddd6fe;border-radius:16px;">
      <p style="margin:0 0 12px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.12em;color:${C.purple};">${escapeHtml(ed.spotlightKicker || 'Watch')} &#9654;</p>
      ${ed.spotlightCtaUrl ? `<a href="${ed.spotlightCtaUrl}" style="text-decoration:none;">` : ''}<img src="${img}" alt="${escapeHtml(ed.spotlightTitle)}" width="536" style="display:block;width:100%;height:auto;border-radius:12px;border:1px solid ${C.border};background-color:${C.white};" />${ed.spotlightCtaUrl ? '</a>' : ''}
      <p style="margin:16px 0 0;font-size:17px;font-weight:800;color:${C.dark};line-height:1.25;">${escapeHtml(ed.spotlightTitle)}</p>
