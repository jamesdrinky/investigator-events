    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-white">
      <div className="relative overflow-hidden bg-[linear-gradient(165deg,#f0f4ff_0%,#e8eeff_25%,#f0e8ff_50%,#f4f0ff_75%,#f8fbff_100%)] pb-10 pt-24 sm:pb-16 sm:pt-32">
        ...
        <div className="container-shell relative text-center">
          ...
          <p className="mx-auto mt-3 max-w-lg text-sm leading-relaxed text-slate-600 sm:text-base">Positions in the private investigation and intelligence sector.</p>
        </div>
      </div>
      ...
      <div className="container-shell py-10 sm:py-14">
        ...
        <div className="grid grid-cols-1 gap-3 sm:flex sm:flex-wrap sm:items-center">
          <select className="field-input w-full text-sm sm:w-auto" value={filterCountry} onChange={(e) => setFilterCountry(e.target.value)}>
            <option value="">All countries</option>
            {countries.map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
          <select className="field-input w-full text-sm sm:w-auto" value={filterType} onChange={(e) => setFilterType(e.target.value)}>
            <option value="">All types</option>
            {JOB_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
          <select className="field-input w-full text-sm sm:w-auto" value={filterSpec} onChange={(e) => setFilterSpec(e.target.value)}>
          ...
          {filtered.map((j) => (
            <div key={j.id} className="rounded-2xl border border-slate-200/60 bg-white p-5 shadow-sm transition hover:shadow-md sm:p-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <a href={`mailto:${j.contact_email}`} className="flex shrink-0 items-center gap-1.5 rounded-full bg-slate-100 px-4 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-200">
