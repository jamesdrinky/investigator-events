...
export default function MyAssociationsPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[linear-gradient(165deg,#f0f4ff_0%,#e8eeff_25%,#f0e8ff_50%,#f4f0ff_75%,#f8fbff_100%)] pt-16 lg:pt-20">
      <div aria-hidden className="pointer-events-none absolute -top-32 -left-24 h-[26rem] w-[26rem] rounded-full bg-[radial-gradient(circle,rgba(20,184,166,0.18),transparent_65%)] blur-3xl" />
      ...
      <div className="container-shell relative max-w-3xl py-6 lg:py-10">
        <span className="inline-flex items-center gap-2 rounded-full border border-emerald-200/60 bg-white/80 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.22em] text-emerald-600 backdrop-blur-sm">
          <span className="inline-block h-1.5 w-1.5 rounded-full bg-emerald-500 shadow-[0_0_0_3px_rgba(16,185,129,0.2)] animate-pulse" />
          Memberships
        </span>
        <h1 className="mt-3 text-[2rem] font-bold leading-[0.95] tracking-[-0.04em] text-slate-950 lg:text-[2.75rem]">
          My{' '}
          <span
            className="inline-block bg-[linear-gradient(92deg,#10b981_0%,#06b6d4_50%,#3b82f6_100%)] bg-[length:200%_100%] bg-clip-text text-transparent"
            style={{ animation: 'gradient-text-cycle 5s ease-in-out infinite' }}
          >
            Associations
          </span>
        </h1>
        <p className="mt-2 text-sm text-slate-600 lg:text-base">Associations you're a member of and their upcoming events</p>
