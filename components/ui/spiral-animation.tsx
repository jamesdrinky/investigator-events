...
class AnimationController {
  ...
  public showProjectedDot(position: Vector3D, sizeFactor: number, colorSeed?: number) {
    ...
    if (position.z > newCameraZ) {
      ...
      if (colorSeed !== undefined) {
        this.ctx.fillStyle = this.getParticleColor(colorSeed);
      }
  ...
  // Brand gradient colours for particles
  private getParticleColor(progress: number): string {
    // Cycle through: blue → cyan → purple → pink
    const p = ((progress * 2) + this.time * 0.3) % 1;
    if (p < 0.25) {
      const t = p / 0.25;
      return `rgb(${Math.round(59 + (34 - 59) * t)}, ${Math.round(130 + (211 - 130) * t)}, ${Math.round(246 + (238 - 246) * t)})`;
    } else if (p < 0.5) {
      const t = (p - 0.25) / 0.25;
      return `rgb(${Math.round(34 + (168 - 34) * t)}, ${Math.round(211 + (85 - 211) * t)}, ${Math.round(238 + (247 - 238) * t)})`;
    } else if (p < 0.75) {
      const t = (p - 0.5) / 0.25;
      return `rgb(${Math.round(168 + (236 - 168) * t)}, ${Math.round(85 + (72 - 85) * t)}, ${Math.round(247 + (153 - 247) * t)})`;
    } else {
      const t = (p - 0.75) / 0.25;
      return `rgb(${Math.round(236 + (59 - 236) * t)}, ${Math.round(72 + (130 - 72) * t)}, ${Math.round(153 + (246 - 153) * t)})`;
    }
  }
  ...
  public render() {
    const ctx = this.ctx;
    if (!ctx) return;
    // Dark navy background matching hero
  ...
  private drawTrail(t1: number) {
    for (let i = 0; i < this.trailLength; i++) {
      ...
      const trailProgress = i / this.trailLength;
      this.ctx.fillStyle = this.getParticleColor(trailProgress);
      this.ctx.lineWidth = sw;
...
class Star {
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (q > 0) {
      ...
      controller.showProjectedDot(position, dotSize, this.spiralLocation + this.angle / (Math.PI * 2));
    }
  }
}
