          ...
          {members.slice(0, 4).map((member, i) => (
            <Reveal key={member.id} delay={i * 60}>
              ...
              >
                ...
                <p className="mt-3 text-sm font-bold text-slate-900 sm:text-base">{member.fullName}</p>
                {member.headline && (
                  <p className="mt-0.5 line-clamp-1 text-[11px] text-slate-500 sm:text-xs">{member.headline}</p>
                )}
                {member.country && (
                  <p className="mt-2 inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-700">
                    📍 {member.country}
                  </p>
                )}
              </Link>
