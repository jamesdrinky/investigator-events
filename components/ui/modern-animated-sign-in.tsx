  ...
  return (
    <section className='max-md:w-full flex flex-col gap-4 w-96 mx-auto'>
      ...
      <form onSubmit={handleSubmit}>
        <section
          className={`grid grid-cols-1 ${fieldPerRow > 1 ? `md:grid-cols-${fieldPerRow}` : ''} mb-4`}
        >
          {fields.map((field) => (
        ...
        >
          <button
            className='bg-gradient-to-br relative group/btn from-zinc-200
            to-zinc-200 block w-full text-black
            rounded-md h-10 font-medium shadow-[0px_1px_0px_0px_#ffffff40_inset,0px_-1px_0px_0px_#ffffff40_inset] outline-hidden hover:cursor-pointer'
            type='submit'
          >
            {submitButton} &rarr;
            <BottomGradient />
...
const AuthTabs = memo(function AuthTabs({
  ...
  return (
    <div className='flex max-lg:justify-center w-full md:w-auto'>
      <div className='w-full lg:w-1/2 min-h-[100dvh] flex flex-col justify-center items-center max-lg:px-[5%] sm:max-lg:px-[10%] py-8'>
        <AnimatedForm
          {...formFields}
